...
def transform_dim_cancellation(s3_bucket: str):
    ...
    df_final = df_final.withColumn("DimOrderKey", f.hash(f.lit("Cancellation"), f.col("InvoiceNo"), f.col("SourceSystem")))
    df_final.select(
        "DimOrderKey",
        "InvoiceNo",
        "InvoiceDate",
    ...
    df_final.write.mode("overwrite").parquet(f"s3a://{s3_bucket}/silver/dim_cancellation/v1")
    spark.stop()
